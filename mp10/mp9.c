...
void up_update(graph_t* g, heap_t* h, int32_t index);
    ...
    if(nnum*4+1 > p->n_nodes-1){
        if(in_range(loc,p->node[nnum].x,p->node[nnum].y_left)){
            ...
            if(vs->count >= vs->id_array_size){
                void* temp_ptr;
                temp_ptr = realloc(vs->id,2*vs->id_array_size*sizeof(*vs->id));
                vs->id_array_size *= 2;
                if(temp_ptr == NULL){
                    // if allocation failed, simply skip
                    return;
                }
                vs->id = temp_ptr;
            }
            ...
            for(int32_t i = 0;i<vs->count;i++){
                if(vs->id[i] > vs->id[vs->count]){
                    insert_index = i;
                    break;
                }
            }
            for(int32_t i=vs->count;i>insert_index;i--){
                swap(&vs->id[i],&vs->id[i-1]);
            }
            vs->count++;
            // end of revison for mp10
        }   
        return;
    }
          ...
          path_t* path)
{
    ...
    if(g->vertex[min_dest_id].pred != UNDEFINED){   
        ...
        while(current_id != UNDEFINED){
            ...
            // mp9 code, abandon now
            // if(path->n_vertices  > MAX_PATH_LENGTH){
            //     return 0;
            // }

            // writing the shortest path to the array
            //path->id[path->n_vertices] = current_id;      //Revised for mp10
            path->n_vertices++; 
            current_id = g->vertex[current_id].pred;
        }
        
        // above while loop calculate the size
        
        // mp10 code
        void* temp_ptr;
        temp_ptr = malloc(path->n_vertices * sizeof(path->id[0]));
        if(temp_ptr == NULL){
            return 0;   // allocation fails, return 0
        }
        path->id = temp_ptr;
